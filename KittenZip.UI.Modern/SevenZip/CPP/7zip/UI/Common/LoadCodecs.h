...
---------------
  ...
  There are two types of executables in KittenZip:

  1) Executable that uses external plugins must be compiled
     with EXTERNAL_CODECS defined:
       - KittenZip.Console.exe, KittenZip.Windows.exe, KittenZip.Modern.FileManager.exe

     Note: EXTERNAL_CODECS is used also in CPP/7zip/Common/CreateCoder.h
           that code is used in plugin module (KittenZip.Core.dll).

  2) Standalone modules are compiled without EXTERNAL_CODECS:
    - SFX modules: KittenZip.Core.Windows.sfx, KittenZip.Universal.Console.sfx
