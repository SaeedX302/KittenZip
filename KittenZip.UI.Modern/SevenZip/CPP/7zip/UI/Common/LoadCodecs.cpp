...
---------------
  ...
  It stops the checking, if it can find any of the following items:
    - KittenZip.Core.dll file
    - "Formats" subdir
    - "Codecs"  subdir
  The order of check:
    1) directory of client executable
    2) WIN32: directory for REGISTRY item [HKEY_*\Software\KittenZip\Path**]
...
-------------
  ...
  KittenZip doesn't use that feature. KittenZip uses the scheme:
    - client application without internal plugins.
    - KittenZip.Core.dll module contains all (or almost all) plugins.
      KittenZip.Core.dll can use codecs from another plugins, if required.
*/
...
static CFSTR const kMainDll =
  #ifdef _WIN32
    FTEXT("KittenZip.Core.dll");
  #else
    FTEXT("KittenZip.Core.so");
  #endif


#ifdef _WIN32

static LPCTSTR const kRegistryPath = TEXT("Software") TEXT(STRING_PATH_SEPARATOR) TEXT("KittenZip");
...
HRESULT CCodecs::LoadDll(const FString &dllPath, bool needCheckDll, bool *loadedOK)
{
  ...
  if (lib.Lib.Load(dllPath))
  {
    ...
    */
    ...
    // plugins can use non-KittenZip dlls, so we silently ignore non7zip DLLs
    /*
    if (!used)
    {
      CCodecError &error = Errors.AddNew();
      error.Path = dllPath;
      error.Message = "no KittenZip code";
    }
    */
  }
  else
...
HRESULT CCodecs::Load()
{
  ...
  // printf("\nLoad codecs \n");
    ...
    {
      bool loadedOK;
      RINOK(LoadDll(baseFolder + L"KittenZip.Codecs.dll", false, &loadedOK));
      if (!loadedOK)
        MainDll_ErrorPath = L"KittenZip.Codecs.dll";
    }
    // **************** KittenZip Modification End ****************
    {
      bool loadedOK;
