      ...
      UINT commandIDFirst, UINT commandIDLast, UINT flags)
{
  ...
  */
  ...
  if (ci.Cascaded.Val)
  {
    ...
    MyAddSubMenu(_commandMap, kMainVerb, menu, indexMenu++, currentCommandID++, (UString)"KittenZip",
  ...
  if (
      // !_isMenuForFM && // 21.04: we don't hide CRC SHA menu in KittenZip
      needCrc
...
HRESULT CZipContextMenu::InvokeCommandCommon(const CCommandMapItem &cmi)
{
  ...
  {
    ::MessageBoxW(0, L"Error", L"KittenZip", MB_ICONERROR);
  }
  return S_OK;
}
...
STDMETHODIMP CZipExplorerCommand::GetIcon(IShellItemArray * /* psiItemArray */, LPWSTR *ppszIcon)
{
  ...
  *ppszIcon = NULL;
  ...
  // imageName += "KittenZip.Windows.exe";
  imageName += "KittenZip.dll";
  // imageName += ",190";
