...
static LPCTSTR const kSvenZipSetFolderFormat = TEXT("KittenZip::SetTargetFolder");
...
STDMETHODIMP CDropSource::QueryContinueDrag(BOOL escapePressed, DWORD keyState)
{
  ...
  if ((keyState & MK_LBUTTON) == 0)
  {
    ...
    if (needExtract)
    {
      ...
      // If you know better way to solve the problem, please notify KittenZip developer.
...
void CPanel::OnDrag(LPNMLISTVIEW /* nmListView */)
{
...

  {
    ...
    // names variable is     USED for drag and drop from KittenZip to Explorer or to KittenZip archive folder.
    // names variable is NOT USED for drag and drop from KittenZip to KittenZip File System folder.
  ...
  We want to use MOVE_OPERATION for extracting from archive (open in KittenZip) to Explorer:
  ...
  if DropTarget is KittenZip window, then KittenZip's
...
/*
  There are 2 different actions, when we drag to KittenZip:
  1) Drag from any external program except of Explorer to "KittenZip" FS folder.
     We want to create new archive for that operation.
  2) all another operation work as usual file COPY/MOVE
    - Drag from "KittenZip" FS to "KittenZip" FS.
        COPY/MOVE are supported.
    - Drag to open archive in KittenZip.
        We want to update archive.
        We replace COPY to MOVE.
    - Drag from "KittenZip" archive to "KittenZip" FS.
        We replace COPY to MOVE.
*/
