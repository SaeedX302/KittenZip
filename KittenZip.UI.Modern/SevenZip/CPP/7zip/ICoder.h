...
/*
  ...
  Parameters:
    ...
    if (inSize != NULL)
    {
      Encoders in KittenZip ignore (inSize).
      Decoder can use (*inSize) to check that stream was decoded correctly.
      Some decoder in KittenZip check it, if (full_decoding mode was set via ICompressSetFinishMode)
    }
    ...
    if (outSize != NULL)
    {
      Encoders in KittenZip ignore (outSize).
