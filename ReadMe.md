# ![KittenZip](Assets/KittenZip.png) KittenZip

[![GitHub Actions Build Status](https://github.com/M2Team/KittenZip/actions/workflows/BuildBinaries.yml/badge.svg?branch=master&event=push)](https://github.com/M2Team/KittenZip/actions/workflows/BuildBinaries.yml?query=event%3Apush+branch%3Amaster)
[![Total Downloads](https://img.shields.io/github/downloads/M2Team/KittenZip/total)](https://github.com/M2Team/KittenZip/releases)
...
[![Latest Version - Release Channel](https://img.shields.io/github/v/release/M2Team/KittenZip?display_name=release&sort=date&color=%23a4a61d)](https://github.com/M2Team/KittenZip/releases/latest)
[![Latest Version - Preview Channel](https://img.shields.io/github/v/release/M2Team/KittenZip?include_prereleases&display_name=release&sort=date&color=%23a4a61d)](https://github.com/M2Team/KittenZip/releases)

[![Latest Release Downloads - Release Channel](https://img.shields.io/github/downloads/M2Team/KittenZip/latest/total)](https://github.com/M2Team/KittenZip/releases/latest)
[![Latest Release Downloads - Preview Channel](https://img.shields.io/github/downloads-pre/M2Team/KittenZip/latest/total)](https://github.com/M2Team/KittenZip/releases)

[![Download KittenZip from SourceForge mirror](https://img.shields.io/badge/SourceForge-Download-orange)](https://sourceforge.net/projects/KittenZip/files/latest/download)
...
KittenZip is an open source file archiver intended for the modern Windows
...
If you want to sponsor the development of KittenZip, please read the document of
[KittenZip Sponsor Edition](Documents/SponsorEdition.md). It's free for all
KittenZip source code repository contributors.

In general, KittenZip Sponsor Edition is more like contributor's edition, but
we provide a way to buy the honor and make someone look like a source code
contributor, because KittenZip is always a community-friendly open-source project.
...
Besides KittenZip.org, KittenZip's official project pages are only hosted on GitHub,
...
- Provide 7-Zip execution alias for helping users to migrate to KittenZip.
...
- Provide additional security mitigations.
  ...
  - Enable disabling child process creation for KittenZip CLI and Self Extracting
...
## Differences between KittenZip and KittenZip Classic

KittenZip plans to have two distribution flavors called KittenZip and KittenZip
...
- KittenZip Classic (**Work In Progress**)
...
> [!CAUTION]
> KittenZip can be used as portable version if you use the official portable
...
> using KittenZip in portable mode.
...
> from the KittenZipPackage project output. For example, if you are using the
> files extracted from the MSIX package of KittenZip like Scoop package manager
> does. We hugely recommend you to use the official release packages of KittenZip.
> For example, WinGet package manager uses KittenZip official release package to
> install KittenZip.

## System Requirements

Here are the system requirements starting with KittenZip 6.0:
...
In general, KittenZip follows the 2025 baseline defined by Kenji Mouri's document,
...
(64-bit) starting with KittenZip 6.0, it gives users chances to use Self
...
We chose Windows as the major platform for the KittenZip project supported
...
- The modern flavor of KittenZip won't support non-Windows unless someone ported
  WinRT XAML to POSIX. So, non-Windows support policy is only for KittenZip Core
  and KittenZip Classic.
...
> [!NOTE]
> In theory, KittenZip Classic can run on Windows 10 (Build 10240) or later if
...
Here are some available installation methods for KittenZip.

### Microsoft Store

This is the recommended way to install KittenZip.

Search and install `KittenZip` in Windows Store for the stable release, and `KittenZip
Preview` for the preview release.

Also, you can also click the Microsoft Store link you needed.

- [KittenZip](https://www.microsoft.com/store/apps/9N8G7TSCL18R)
- [KittenZip Preview](https://www.microsoft.com/store/apps/9NZL0LRP1BNL)

### MSIX Package

You can download the MSIX package and the corresponding XML license file from
[GitHub Releases](https://github.com/M2Team/KittenZip/releases). After you have
finished downloading the relevant files, you can use one of the methods
described below to install KittenZip:

> [!NOTE]
> All required dependencies are bundled with KittenZip's MSIX package, as users
> without access to the Microsoft Store may have difficulty obtaining them
> separately. This is intended to make KittenZip more robust and easier to deploy
...
to install KittenZip.

> [!CAUTION]
> If KittenZip is installed using this method without an Internet connection, you
> must connect to the Internet and launch it once so that Windows can acquire
> the required license. Otherwise, KittenZip may not launch properly.
...
If your downloaded KittenZip release provides the XML license file, you can use
the following command to install KittenZip for all users.
...
If you want to uninstall KittenZip if you installed it with this method, you can
use the following command to uninstall KittenZip.

If you open the PowerShell as Administrator, you can execute:

> Get-AppxPackage -Name *40174MouriNaruto.KittenZip* -AllUsers | Remove-AppxPackage -AllUsers -Confirm

#### Command Line (Without license file)

If your downloaded KittenZip release doesn't provide the XML license file, or you
don't want to install KittenZip for all users, you can use the following command
to install KittenZip for the current user.
...
- If you can't find KittenZip in the context menu, please restart all File
...
- Due to the issues in Desktop Bridge file system virtualization, you are
  unable to use KittenZip in the Safe Mode of Windows.
- Due to the policy from Microsoft Store, KittenZip is unable to disable Desktop
...
- Due to the Microsoft Store limitations, KittenZip 1.2 and later won't support
  languages not mentioned in
  https://docs.microsoft.com/en-us/windows/uwp/publish/supported-languages.
- If you turn off the Windows Firewall, KittenZip may fail to install.
  (https://github.com/M2Team/KittenZip/issues/204)
...
- Due to the System Settings limitations, starting with Windows 11+
  ...
  page for file association for KittenZip.
  (https://learn.microsoft.com/en-us/windows/uwp/launch-resume/launch-default-apps-settings)
- Due to the Microsoft Store Licensing API implementation, you will find KittenZip
  will try to access the Internet beginning with KittenZip 3.0 because KittenZip
  needs to use that to check the Sponsor Edition addon licensing status.
- For optimizing the user experience, KittenZip will only check the Sponsor
  Edition addon licensing status at the first time your launch KittenZip File
...
- [KittenZip Sponsor Edition](Documents/SponsorEdition.md)
- [KittenZip Release Notes](Documents/ReleaseNotes.md)
- [KittenZip Preview Release Notes](Documents/ReleaseNotesPreview.md)
- [KittenZip Upstream Synchronization Status](Documents/UpstreamSynchronization.md)
- [KittenZip Group Policy Administrative Templates (ADMX/ADML)](Documents/PolicyDefinitions)
- [KittenZip Accessibility Conformance Report for Section 508](Documents/Section508)
